...
/*
  These should be common use cases that require knowledge of the registry
  layout.
  So these will be specific to XL-320

  In general the return value is the "success" of the tx/rx, while the actual
  queried
...
/*
  ...
  from the dynamixels after each command. Can be used to optimize for
  transmission size, etc.
*/
...
bool SetPositionAndVelocity(uint8_t dxl_id, uint16_t position,
                            uint16_t velocity);
