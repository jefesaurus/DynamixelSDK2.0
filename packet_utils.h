...
static inline uint8_t LowBits(uint16_t word) { return word & 0xFF; }

static inline uint8_t HighBits(uint16_t word) { return (word >> 8) & 0xFF; }

static inline uint16_t MakeWord(uint8_t low_bits, uint8_t high_bits) {
