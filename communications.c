...
void TerminateDXL(void) { dxl_hal_close(); }
...
bool RXPacket(uint8_t* received_data, int packet_length,
  ...
  while (bytes_read < packet_length) {
    ...
    bytes_read +=
        dxl_hal_rx(&(received_data[bytes_read]), packet_length - bytes_read);
    if (dxl_hal_timeout() == 1) {
      return false;
    }
  }
  ...
  uint16_t expected_crc =
      UpdateCRC(0, received_data, bytes_read - CHECKSUM_SIZE);
  uint16_t received_crc =
      MakeWord(received_data[bytes_read - 2], received_data[bytes_read - 1]);
